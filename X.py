...
sys.stdout.write('\x1b[1;97m\x1b]2; {×} Crack by Shelly {×} \x07')
...
def banner():
	os.system('clear')
	cetak(nel(f"""______________        ____________     Recode : Shelly Scott\n__  ___/___  /_ _____ ___  /___  /_____  __ Author : Romi\n_____ \ __  __ \_  _ \__  / __  / __  / / /\n____/ / _  / / //  __/_  /  _  /  _  /_/ / \n/____/  /_/ /_/ \___/ /_/   /_/   _\__, /  \n                                  /____/ """,title="INFORMASI"))
...
def Masuk():
	try:
		...
		os.system('clear')
		banner()
		cetak(nel(f"""[01] Login Instagram Via Cookie\n[02] Login Facebook Via Cookie\n[03] Tutorial mendapatkan cookie\n[00] Keluar""",title="LOGIN"))
		while True:
...
class pilihan:
	...
	def menu(self):
		...
		if slut in['',' ']:
			...
			gan = input ("\n%s%s%s ingin crack massal ? y/t%s >%s "%(U,til,O,M,K))
			if gan in[""]:
				...
				print ("\n%s%s %sPastikan daftar teman bersifat publik bro"%(U,til,O))
			...
			print ("\n%s%s %sPastikan target mempunyai pengikut "%(U,til,O))
			idt = input('%s%s %sUsername/Id%s > %s'%(U,til,O,M,K))
			if idt in[""," "]:
				print ('\n%s%s isi yang benar'%(M,til));jeda(2)
			else:
			...
			print ("\n%s%s %sPastikan postingan bersifat publik  "%(U,til,O))
		...
		if unikers in ('Y', 'y'):
			...
			while True:
				...
				if pwx == '':
					...
					print ('%s• %s01%s methode %sfree %s(ngebut rawan cp) '%(U,P,O,M,O))
					print ('%s• %s02%s methode %smbasic %s(slow recomended) '%(U,P,O,P,O))
					print ('%s• %s03%s methode %smobile %s(very slow kaya siput) '%(U,P,O,H,O))
					manual(pwx.split(','))
					break
		elif unikers in ('T', 't'):
			print ('\n%s•%s [ %spilih methode login, silahkan coba satu²%s ]\n'%(U,O,U,O))
			print ('%s• %s01%s methode %sfree %s(ngebut kaya emaklu bawa motor)'%(U,P,O,M,O))
			print ('%s• %s02%s methode %smbasic %s(slow recomended✓) '%(U,P,O,P,O))
			print ('%s• %s03%s methode %smobile %s(lelet kaya keong) '%(U,P,O,H,O))
			self.langsung()
		else:
...
def cek_apk(kukis):
	...
	try:
		for i in range(len(game)):
			print ("\r%s[Apk Aktif]\n    -->%s%s%s%s"%(H,P,[i+1],H,game[i].replace("Ditambahkan pada"," Ditambahkan pada")))
	except AttributeError:
	...
	try:
		for i in range(len(game)):
			print ("\r%s[Non-Aktif]\n    -->%s%s%s%s"%(M,P,[i+1],M,game[i].replace("Kedaluwarsa"," Kedaluwarsa")))
	except AttributeError:
		print ("\r      %s• cookie invalid"%(M))
...
def login():
	...
	if x.get('status')=='success':
		...
		print (' Login berhasil √'%(P,til));jeda(2)
		checkin()
...
def register_device():
	while True:
		if os.path.exists("/data/data/com.termux/files/usr/etc/.license"):
			...
			check = requests.get("https://pastebin.com/raw/JSaq3rSw")
			if key in check.text:
				print("Key anda telah di konfirmasi")
				time.sleep(1.5)
				Masuk()
			else:
				print("Key anda belum di konfirmasi\nSilahkan Beli Ke Wa 6285322904155")
